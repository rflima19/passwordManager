package app.passwordmanager.dao.db;
...
public class SenhaServicoDBDAO implements SenhaServicoDAO {
	...
	public List<SenhaServico> load() {
		// Delega para o método filter(). Se o parâmetro String for nulo o metodo
		// filter() busca todos os registros da tabela.
		...
		try (Connection conn = ConnectionFactory.openConnection()) {
			...
			try (PreparedStatement stmt = conn.prepareStatement(insertSql)) {
				for (SenhaServico senhaServico : senhasServico) {
					...
					stmt.setString(4, this.encrypt(senhaServico.getSenha()));
		...
		try (Connection conn = ConnectionFactory.openConnection()) {
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				try (ResultSet rs = stmt.executeQuery()) {
					...
					// Para cada registro encontrado na tabela, cria um objeto SenhaServico e coloca
					// na lista
					...
					while (rs.next()) {
						...
						senhaServico.setSenha(this.decrypt(rs.getString("SENHA")));
